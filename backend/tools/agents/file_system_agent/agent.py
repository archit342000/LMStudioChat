...
async def flow_fn(agent: Any, instruction: str, **kwargs) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        logger.error(f"FileSystemAgent failed during execution: {e}", exc_info=True)
        try:
            db.add_message(
                chat_id=chat_id,
                role='event',
                content=f'FileSystem Agent failed: {str(e)}',
                parent_id=parent_message_id,
                parent_type='file_system_agent'
            )
        except Exception as db_err:
            logger.error(f"Failed to log file system agent failure event: {db_err}")
        agent.result = f"FileSystem agent failed: {str(e)}"
        yield f"Error: FileSystem agent failed during execution: {str(e)}"