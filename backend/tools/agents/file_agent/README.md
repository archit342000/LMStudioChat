...
| `prompts.py` | System prompts and directives for the file agent. | `FILE_AGENT_SYSTEM_PROMPT` |
| `tools.py` | Internal tool implementations for the agent (RAG, grep, read). | `file_agent_rag_tool`, `grep_uploaded_file_tool`, `read_uploaded_file_tool` |
...
*   **Testing:** Local tests are located in `tests/`. Run with `pytest backend/tools/agents/file_agent/tests/`.
