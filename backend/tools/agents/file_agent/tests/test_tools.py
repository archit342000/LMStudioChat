...
from backend.tools.agents.file_agent.tools import (
    _get_managers,
    file_agent_rag_tool,
...
def test_get_managers():
    with patch('backend.tools.agents.file_agent.tools.get_embedding_model') as mock_get_emb, \
         patch('backend.tools.agents.file_agent.tools.RAGProvider') as mock_rag_provider, \
         patch('backend.tools.agents.file_agent.tools.FileManager') as mock_fm, \
         patch('backend.tools.agents.file_agent.tools.FileRAG') as mock_file_rag:
...
@pytest.mark.anyio
async def test_file_agent_rag_tool_no_results():
    mock_fm = MagicMock()
    mock_fr = MagicMock()
    mock_fr.retrieve_for_file = AsyncMock(return_value=[])
    
    with patch('backend.tools.agents.file_agent.tools._get_managers', return_value=(mock_fm, mock_fr)):
        res = await file_agent_rag_tool("chat_id", "file_id", "query", depth="standard")
...
@pytest.mark.anyio
async def test_file_agent_rag_tool_with_results():
    mock_fm = MagicMock()
    mock_fr = MagicMock()
    mock_fr.retrieve_for_file = AsyncMock(return_value=["res1"])
    
    with patch('backend.tools.agents.file_agent.tools._get_managers', return_value=(mock_fm, mock_fr)):
        res = await file_agent_rag_tool("chat_id", "file_id", "query", depth="deep")
        assert res["success"] is True
