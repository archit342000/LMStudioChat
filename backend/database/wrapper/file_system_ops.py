...
class FileSystemOpsMixin(BaseMixin):
    ...
    def share_fs_file(self, file_system_id: str, chat_id: str = None, workspace_id: str = None, user_id: str = "any_user", permission: str = "read"):
        """Share a file_system with a specific user."""
        def _write():
            conn = make_connection()
            try:
                c = conn.cursor()
                ...
                try:
                    c.execute("""
                        INSERT INTO file_system_permissions (file_system_id, chat_id, workspace_id, user_id, permission, timestamp)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (file_system_id, chat_id, workspace_id, user_id, permission, time.time()))
                    conn.commit()
                    return True
                except:
                    conn.rollback()
        ...
        _write()
        owner_key = f"ws:{workspace_id}" if workspace_id else f"chat:{chat_id}"
        cache_layer.invalidate("file_systems", f"{owner_key}:{file_system_id}:shared")
        return {"success": True}

    def unshare_fs_file(self, file_system_id: str, chat_id: str = None, workspace_id: str = None, user_id: str = "any_user"):
        """Stop sharing a file_system with a specific user."""
        def _write():
            conn = make_connection()
            try:
                c = conn.cursor()
                ...
                try:
                    c.execute("""
                        DELETE FROM file_system_permissions 
                        WHERE file_system_id = ? AND (chat_id = ? OR workspace_id = ?) AND user_id = ?
                    """, (file_system_id, chat_id, workspace_id, user_id))
                    conn.commit()
                    return True
                except:
                    conn.rollback()
    ...
    def get_shared_users(self, file_system_id: str, chat_id: str = None, workspace_id: str = None):
        """Get the list of users a file_system is shared with."""
        def _fetch():
            conn = make_connection()
            try:
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                if workspace_id:
                    c.execute("SELECT user_id, permission, timestamp FROM file_system_permissions WHERE file_system_id = ? AND workspace_id = ?", (file_system_id, workspace_id))
                else:
                    c.execute("SELECT user_id, permission, timestamp FROM file_system_permissions WHERE file_system_id = ? AND chat_id = ?", (file_system_id, chat_id))
                return [dict(row) for row in c.fetchall()]
            finally:
                conn.close()
        owner_key = f"ws:{workspace_id}" if workspace_id else f"chat:{chat_id}"
        return cache_layer.get("file_systems", f"{owner_key}:{file_system_id}:shared", _fetch, ttl=300)
