...
class ResponseCache:
    ...
    def initialize_chat(self, chat_id, ttl_seconds=None, overwrite=True):
        ...
        with self._lock:
            is_new_cache = False
            if overwrite and chat_id in self._cache:
                self._cache[chat_id]['chunks'] = []
                self._cache[chat_id]['sse_chunks'] = []
    ...
    def append_chunk(self, chat_id, chunk_data):
        ...
        """
        if chat_id not in self._cache:
            # Re-initialize without overwriting if it was evicted but task is still running
            self.initialize_chat(chat_id, overwrite=False)
    ...
    # ==================== SSE CHUNKS (TRANSIENT STORE) ====================

    def add_sse_chunk(self, chat_id, parent_message_id, parent_type, chunk_index, chunk_type, content, timestamp=None):
        """
        Store a transient SSE chunk in memory and SSE-WAL.
        These are aggregated and flushed to messages/sub_agent_messages in DB at the end of a turn.
        """
        ...
        # Initialize if not exists (e.g. if we somehow missed it)
        if chat_id not in self._cache:
            self.initialize_chat(chat_id, overwrite=False)

        with self._cache[chat_id]['lock']:
            self._cache[chat_id]['sse_chunks'].append(chunk)
            self._cache[chat_id]['last_updated'] = time.time()

    def get_sse_chunks(self, chat_id, parent_message_id=None, parent_type=None):
        """Retrieve stored SSE chunks for a chat, optionally filtered."""
        if chat_id not in self._cache:
            return []

        with self._cache[chat_id]['lock']:
            chunks = list(self._cache[chat_id]['sse_chunks'])

        if parent_message_id is not None:
            chunks = [c for c in chunks if c['parent_message_id'] == parent_message_id]
        if parent_type is not None:
            chunks = [c for c in chunks if c['parent_type'] == parent_type]

        # Sort by parent_message_id then chunk_index to match original DB behavior
        chunks.sort(key=lambda x: (x['parent_message_id'], x['chunk_index']))
        return chunks

    def clear_sse_chunks(self, chat_id):
        """Clear all SSE chunks for a chat session."""
        if chat_id in self._cache:
            with self._cache[chat_id]['lock']:
                self._cache[chat_id]['sse_chunks'] = []

    def delete_sse_chunks(self, chat_id, parent_message_id=None, parent_type=None):
        """Delete SSE chunks for a chat session, optionally filtered."""
        if chat_id not in self._cache:
            return 0
        
        with self._cache[chat_id]['lock']:
            old_len = len(self._cache[chat_id]['sse_chunks'])
            if parent_message_id is None and parent_type is None:
                self._cache[chat_id]['sse_chunks'] = []
            else:
                self._cache[chat_id]['sse_chunks'] = [
                    c for c in self._cache[chat_id]['sse_chunks']
                    if not (
                        (parent_message_id is None or c['parent_message_id'] == parent_message_id) and
                        (parent_type is None or c['parent_type'] == parent_type)
                    )
                ]
            deleted_count = old_len - len(self._cache[chat_id]['sse_chunks'])
            ...
            while True:
                try:
                    item = await asyncio.to_thread(
                        q.get,
                        timeout=config.SUBSCRIBER_POLL_INTERVAL
                    )
                except queue.Empty:
                    # No data arrived within poll interval.
                    # Check if the producing task is still alive.
                    if not task_manager.is_task_running(chat_id):
                        log_event("subscriber_orphan_termination", {"chat_id": chat_id})
                        break
                    continue
    ...
    def cleanup_expired(self, active_chat_ids=None):
        """
        ...
        with self._lock:
            expired = [
                cid for cid in self._cache 
                if self._is_expired(cid) and cid not in active_chat_ids
            ]
            for cid in expired:
        ...
        for entry in chunks:
            ...
            if isinstance(data, str) and data.startswith("data: "):
                try:
                    ...
                    # ROUTING FIX: Only aggregate 'main' chunks into history.
                    # Anything with a parent_type OTHER than 'main' belongs to a sub-agent.
                    if json_data.get('parent_type', 'main') != 'main':
                        continue
