...
def row_read_lock(table: str, row_id: str):
    ...
    start_time = time.time()
    # WAL wait loops removed to prevent 10s deadlock/stutter
    # SQLite WAL mode natively handles concurrent reading/writing.
...
cache_layer.register_flush_callback("file_systems", lambda: flush_wal("file_systems"))
logger.debug("[DB] Registered flush callback for 'file_systems'")
cache_layer.register_flush_callback("file_system_versions", lambda: flush_wal("file_system_versions"))
logger.debug("[DB] Registered flush callback for 'file_system_versions'")
cache_layer.register_flush_callback("file_system_permissions", lambda: flush_wal("file_system_permissions"))
logger.debug("[DB] Registered flush callback for 'file_system_permissions'")
